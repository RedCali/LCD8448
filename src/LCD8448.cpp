/*
 * LCD Display Library for the 8448 Nokia display
...
void LCD8448::draw_bmp_pixel(uint8_t X, uint8_t Y, unsigned char *map) {
    draw_bmp_pixel(X, Y, map, 84, 48);
}

void LCD8448::draw_bmp_pixel(uint8_t X, uint8_t Y, unsigned char *map, uint8_t Pix_x, uint8_t Pix_y) {
    unsigned int i, n;
    unsigned char row;
...
void LCD8448::write_string(uint8_t X, uint8_t Y, char *str, uint8_t mode) {
    set_XY(X, Y);
    while (*str) {
        write_char(*str, mode);
        str++;
...
void LCD8448::write_char(unsigned char c, uint8_t mode) {
    ...
    for (line = 0; line < 6; line++) {
        ...
        writeData((mode == MENU_NORMAL) ? ch : (ch ^ 0xff));  // MENU_NORMAL=0,True:return ch;False:return ch^0xff
    }
}

void LCD8448::write_string_big(uint8_t X, uint8_t Y, char *str, uint8_t mode) {
    while (*str) {
...
void LCD8448::write_char_big(uint8_t X, uint8_t Y, unsigned char ch, uint8_t mode) {
    unsigned char i, j;
    unsigned char *pFont;
...
/**************************************************************************************/
void LCD8448::write_chinese(uint8_t X, uint8_t Y, unsigned char *c, uint8_t ch_with, uint8_t num, uint8_t line, uint8_t row) {
    unsigned char i, n;
...
void LCD8448::prop_write_string(uint8_t X, uint8_t Y, char *str, uint8_t mode) {
    set_XY(X, Y);
    while (*str) {
...
unsigned char LCD8448::prop_write_char(char c, uint8_t mode) {
    ...
    for (line = line_s; line < line_e + 1; line++) {
        ...
        writeData((mode == MENU_NORMAL) ? ch : (ch ^ 0xff));
    }
    writeData((mode == MENU_NORMAL) ? 0 : 0xff);
    return ((unsigned char)(line_e + 2 - line_s));
}
/*************************************************************************************/
void LCD8448::write_number_big(uint8_t X, uint8_t Y, int zahl, uint8_t komma, uint8_t stellen, uint8_t mode) {
    unsigned char lauf = 0;
    char zahlimarray[stellen + 2];

    if (komma > 0)
        stellen += 1;

    while (lauf < stellen) {
        zahlimarray[stellen - lauf - 1] = '0' + zahl % 10;
        lauf++;
        if ((lauf == komma - 1) && komma != 0) {
            zahlimarray[stellen - lauf - 1] = '.';
            lauf++;
        }
        zahl /= 10;
    }
    zahlimarray[stellen] = '\0';

    write_string_big(10, 1, zahlimarray, MENU_NORMAL);
}
void LCD8448::write_number_big2(uint8_t X, uint8_t Y, uint8_t zahl) {
    unsigned char *ptr = (unsigned char *)font20_24;
    for (char i = 0; i < 3; i++) {
        set_XY(X, i + Y);
        for (char j = 0; j < 20; j++)
            writeData(pgm_read_byte(ptr + j + i * 320 + zahl * 20));
    }
}
/*************************************************************************************/
...
void LCD8448::vd_write_char(uint8_t X, uint8_t Y, char c, uint8_t mode) {
    unsigned char line;
    unsigned char *pFont;
    unsigned char ch1;
    ...
    if (Y % 8 == 0) {
        for (line = 0; line < 6; line++) {
            ...
            vd_set_pixel_byte(X + line, Y / 8, (mode == MENU_NORMAL) ? ch1 : (ch1 ^ 0xff));
        }
    } else {
        unsigned char ch2;
        ...
        for (line = 0; line < 6; line++) {
            ...
            ch2 = (ch1 << posi);
            ch1 = (ch1 >> (8 - posi));
            vd_set_pixel_byte(X + line, Y / 8, (mode == MENU_NORMAL) ? ch2 : (ch2 ^ (0xff << posi)));
            if (Y != 5) {
                vd_set_pixel_byte(X + line, Y / 8 + 1, (mode == MENU_NORMAL) ? ch1 : (ch1 ^ (0xff >> (8 - posi))));
            }
        }
    }
}

void LCD8448::vd_write_char_v(uint8_t X, uint8_t Y, char c, uint8_t mode) {
    unsigned char ch1;
    unsigned char *pFont;
    ...
    for (unsigned char i = 0; i < 8; i++) {
        ...
        vd_set_pixel_byte_any(X + i, Y, (mode == MENU_NORMAL) ? ch1 : (ch1 ^ 0xfc));
    }
}

void LCD8448::vd_write_string(uint8_t X, uint8_t Y, char *s, uint8_t mode) {
    while (*s) {
        vd_write_char(X, Y, *s, mode);
        X += 6;
        s++;
    }
}

void LCD8448::vd_write_string_v(uint8_t X, uint8_t Y, char *s, uint8_t mode) {
    while (*s && Y > 0) {
        vd_write_char_v(X, Y, *s, mode);
        Y -= 6;
        s++;
    }
}
...
void LCD8448::vd_write_line(uint8_t X0, uint8_t Y0, uint8_t X1, uint8_t Y1) {
    signed char dx = fabs(X1 - X0), sx = X0 < X1 ? 1 : -1;
    signed char dy = -fabs(Y1 - Y0), sy = Y0 < Y1 ? 1 : -1;
    int err = dx + dy, e2;
...
void LCD8448::vd_write_framework(char *head, uint8_t mode) {
    ...
    //	vd_battery(10,5,9,MENU_NORMAL);
    //	vd_write_line(1,38,83,38);
    //	vd_write_string(21,39,"22%",MENU_NORMAL);
}

void LCD8448::vd_alert(char *text) {
    ...
    vd_write_string(offset, 2 * 8, text, MENU_HIGHLIGHT);

    draw_bmp_pixel(0, 0, virtuelldisp, 84, 48);
}

void LCD8448::vd_question(char *question, uint8_t aktiv) {
    for (uint16_t i = 0; i < 504; i++) {
        if (i % 2)
            virtuelldisp[i] |= 0x55;
    }
    ...
    vd_write_string(offset, 2 * 8, question, MENU_HIGHLIGHT);
    ...
    vd_write_string(15, 4 * 8, " NO ", (aktiv == 1) ? MENU_HIGHLIGHT : MENU_NORMAL);
    vd_write_string(42, 4 * 8, " YES ", (aktiv == 2) ? MENU_HIGHLIGHT : MENU_NORMAL);
